...
# 函数类别
funcClass = 'TradeMid1Sub'

tradePath = r"D:\Test\逐笔\L2Transaction_{year}{month}\{year}-{month}-{day}\{codeID}.pkl"
...
def SyntheticTradeMid1Sub_worker(readFunc: Callable,
                                 filePath: str,
                                 **kwargs):
    if calFuncs != {}:
...
def onCallFunc(dataParams: Iterable, readFunc: Callable):
    ...
    for sample in dataParams:
        ...
        TradePath = tradePath.format(year=year,
                                     month=month,
                                     day=day,
                                     codeID=codeID)
        try:
            data = readFunc(TradePath)
        ...
        if flag >= 1000 or flag == len(dataParams):
            ...
            print(f"{funcClass} {pid:<5}: {flag:>4}/{len(dataParams):>3}, 耗时{round(time.time() - sta, 4)}")
