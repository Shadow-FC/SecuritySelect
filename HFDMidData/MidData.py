...
class MidData(object):
    Path_Mapping = {"Trade": "B:\\合成数据\\逐笔1min\\逐笔1min",  # 高频分钟数据
                    "Depth": "B:\\合成数据\\十档1min\\十档一分钟样本外"  # 高频十档分钟数据
                    }
    ...
    def __init__(self, local_path: str, file_name: str):
        ...
        self.file_ID_checked = collections.defaultdict(list)
    ...
    def get_file_path_list(self, func_name: str) -> List[str]:
        ID = self.get_json(self.json_name_ID + self.file_name).get(func_name, [])
    ...
    def process_data(self, func_name: str, func: Callable):
        filePathList = self.get_file_path_list(func_name)
        data_set, ID_checked, res_list, flag, process = [], [], [], 0, 0
